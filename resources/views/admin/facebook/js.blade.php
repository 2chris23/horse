    ...
    <script>
        ...
        function EliminarPost(id) {
            swal({
                ...
                form.append('id', id);
                axios.post("{!! $rutaborrar !!}", form).then(function (response) {
        ...
        function limpiarModalCaballo() {
            ...
            $('.ident').html('');
            $('.deletepost').remove();
            $(modal).find('.dp2').val("{!! Funciones::AjustarFechaFormatoMaterial() !!}");
            $(modal).find('.sms').val('');
        ...
        function ClickAndClear(el) {
            limpiarModalCaballo();
            if (el == 'ph') {
                $('.hdt').addClass('hidden-xs-up');
            } else {
                $('.hdt').removeClass('hidden-xs-up');
            }
            $("#" + el).click();
        };

        function BorrarDatosFbAdmin() {
            swal({
            ...
            $('#calendar').fullCalendar({
                ...
                events: [
...

                    {
                        ...
                        linkc: "{!! $linkr !!}",
                        @if($disable == 0)
                        borrar: "<a href='#!' class='btn btn-warning deletepost m-r-10' onclick='EliminarPost({!! $el->id !!})'> {!! Funciones::ReemplazarApostrofe(trans('facebook.borrar')) !!} </a>",
                        @else
                        borrar: "",
                        @endif
                        sms: "{{ $mensaje }}",
