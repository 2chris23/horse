...
// UOU Selects
  ...
  $.fn.uouCustomSelect = function () {
    ...
    if ($select.hasClass('pais')) {
      $select.on('change', function () {

          var s = $('#state');
console.dir(s);
          s.parent().find('.trigger').remove();
          s.parent().find('.select-clone').remove();
          s.parent().find('.value-holder').remove();
          s.parent().removeClass('uou-custom-select');
    ...
    function CambioEstado1() {
        ...
        $('#state1').find("option:eq(0)").html("Por favor espera...");
        ajaxCall(data, UrlEstado, function (data) {
            $('#state1').html('').find("option:eq(0)").html(data.text);
            var _select = $('<select>');
            var datos = data.data;
            $.each(datos, function (key, val) {
                _select.append($('<option>').val(val.id).html(val.name));
            });
            $('#state1').append(_select.html());
            if ($('#country').val() != null || $('#country').val() != 0) {
                $('#state1').val(edo).trigger('change');
            }
            $('#state').uouCustomSelect()
console.dir($('#state'));
