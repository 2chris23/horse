...
{{--
                                    ...
                                    <div class="col-xs-10 col-sm-10 col-md-6">
                                        <select class=" form-control" data-style="btn-primary"
                                            ...
                                            @foreach(trans('horse.raza') as $k=>$v)
                                                <option data-tokens="{!! $k !!}"
                                                        value="{!! $k !!}"
                                                        @if($horse->raza == $k) selected @endif>{!! $v !!}</option>
                                                                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 text-xs-center cubris hidden-xl-down">
                                <div class="form-group row">
                                    <label class="col-6 col-xs-6 col-xs-6 col-md-3 col-lg-3 col-form-label text-right">
                                        {!! trans('horse.cubri') !!}:
                                    </label>
                                    <div class="col-xs-10 col-sm-10 col-md-6">
                                        <input type="text" placeholder="{{trans('horse.placeholder.price')}}"
                                               id="cubri"
                                               name="cubri"
                                               value="{{Funciones::AjustarNumeroMil($horse->ObtenPrecioCubricionMoneda())}}"
                            ...
                            <textarea name="input_stud_description" id="input_stud_description">
                            ...
                            <div class="offset-3 col-6  text-center">
                                <div class="row">
                                    <div class="col-xs-6 col-sm-6 col-md-6 col-lg-4">
                                        <a href="#" onclick="savedata()" class=" btn btn-block btn-success glow_button">{!! trans('users.save') !!}</a>
                                    </div>
                                </div>
                            </div>
...
@section('bottomjs')
        ...
        function envio(form, url) {
            axios.post(url, form)
                .then(function (response) {
                    ...
                    {{-- BORRA //var s = $.parseJSON(data);--}}

                    {{-- BORRA //$('#video').append(s.el);--}}
 ...
 .catch(function (error) {
             ...
             {{-- BORRA //var err = eval(xhr.responseText.sms);--}}
                    var e = error;
                    console.dir(e);
                    var v = e.message;
                    swal({
        ...
        function savedata() {
            ...
            {{-- BORRA //$('.save').on('click', function (e) {

            BORRA //e.preventDefault();

            BORRA //DisableElement($('.save'));

             BORRA //DisableElement($('.cancel'));
            --}}
            ...
            {{-- BORRA //EnableElement($('.save'), true);--}}

            {{-- BORRA //EnableElement($('.cancel'), true);--}}
            ...
            axios.post('{!! route('horse.store') !!}', form)
                .then(function (response) {
                    ...
                    {{-- BORRA //$('.fileinput-upload-button').click();--}}



                    {{-- BORRA //$('.btn-drp-caballo').click();--}}
                    swal(
                        '{!! Funciones::ReemplazarApostrofe(trans('users.applychange')) !!}',
                        r.sms,
                        'success'
 ...
 .catch(function (error) {
        ...
        function savevideo(url) {
            var form = new FormData();
                    ...
                    {{-- BORRA //var description = $('#input_stud_description');--}}
